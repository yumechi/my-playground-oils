...
# =============================================================================
echo ""
echo "4. 静的パース (Single-pass parsing)"
echo "   Bash: bash -n で構文チェック可能だが、パースと実行を交互に行う"
echo "   OSH:  スクリプト全体を事前に単一パスでパース"
echo ""

# 重要な違い:
# Bash はパースと実行を交互に行うため、実行されないブロック内の
# 構文エラーを検出できない場合がある
#
# 例: 以下のコードで bash -n は内部のエラーを検出しない
# if false; then
#     echo ${foo.}      # 不正な変数展開
#     $(echo hi >)      # 不完全なリダイレクト
# fi
#
# OSH は全体を事前にパースするため、上記のエラーも検出できる
#
# 参考: "Parsing Bash is Undecidable" - https://www.oilshell.org/blog/2016/10/20.html

echo "   OSH は実行されないコードブロック内のエラーも検出します"
echo "   (bash -n では検出できない場合がある)"
